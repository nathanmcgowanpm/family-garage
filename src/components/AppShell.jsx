 ...
 * Mobile (<768px):
 *   - Full-width content
 *   - AppHeader at top (via mobileHeader prop)
 *   - BottomNav at bottom (via mobileNav prop)
 *
 * Desktop (≥768px):
 *   - Left sidebar (240px):
 *     - Wordmark (top)
 *     - Nav
 *     - (spacer)
 *     - Fleet chips + Add vehicle (bottom area)
 *     - Account block (very bottom)
 *   - Main content column, max ~640px, centered
 */
  ...
  if (isDesktop) {
    return (
      ...
      >
        ...
        <main style={{ minHeight: '100vh', padding: '40px 24px', position: 'relative', zIndex: 1 }}>
...
function Sidebar({ screen, onNavigate, vehicles, activeVehicle, onSelectVehicle, onAddVehicle, onOpenAccount, user }) {
  return (
    <aside
      style={{
    ...
    >
      ...
      <button
        ...
        style={{
          ...
          textAlign: 'left', fontFamily: 'var(--font-display)', fontSize: 14,
      ...
      {/* Spacer pushes fleet + account to the bottom */}
      <div style={{ flex: 1 }} />
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          <div
            style={{
              ...
              color: 'var(--color-text-secondary)', padding: '0 8px', marginBottom: 4,
            }}
          >
            Fleet
          </div>
          {vehicles.map((v, i) => {
            ...
            const nickname = v.nickname || extractNickname(v.name)
      ...
      {onOpenAccount && user && (
        ...
        >
          ...
          <Icon name="expand_less" style={{ fontSize: 16, color: 'var(--color-text-tertiary)' }} />
        </button>
      )}
    </aside>
  )
